...
/**
 * The Class BiclustersSensitivityMatrix.
 */
public class BiclustersSensitivityMatrix extends AbstractSimilarity{

	/** The name. */
	// Sensitivity inclusion score sen of biclusters A and B is computed sen=|AnB|/|A|
	public static String NAME="biclustersensitivitysimilaritymatrix";

	/**
	 * Instantiates a new biclusters sensitivity matrix.
	 */
	public BiclustersSensitivityMatrix() {
		super();
	}
	 ...
	 */
	public BiclustersSensitivityMatrix(AnalysisTypeDimension dimension) {
		super(dimension);
	}
	 ...
	 * @param dimension the dimension
	 */
	public BiclustersSensitivityMatrix(BiclusterList set1, BiclusterList set2, AnalysisTypeDimension dimension) {
	 ...
	 * @param set2 the set 2
	 */
	public BiclustersSensitivityMatrix(BiclusterList set1, BiclusterList set2) {
		super(set1, set2);
	}
	 ...
	 *
	 * @return the biclusters sensitivity matrix
	 */
	public BiclustersSensitivityMatrix calculateOnlyGenes(){
	 ...
	 * @return the biclusters sensitivity matrix
	 */
	public BiclustersSensitivityMatrix calculateOnlyConditions(){
	 ...
	 * @return the biclusters sensitivity matrix
	 */
	public BiclustersSensitivityMatrix calculateBoth(){
	...
	protected double[][] calculateIndexMatrix(ArrayList<ArrayList<String>> set1, ArrayList<ArrayList<String>> set2) {
		 return calculateSensitivityIndexMatrix(set1, set2);
	}
    

	/**
	 * Calculate sensitivity index matrix.
	 *
	 * @param set1 the set 1
	 * @param set2 the set 2
	 * @return the double[][]
	 */
	public static double[][] calculateSensitivityIndexMatrix(ArrayList<ArrayList<String>> set1, ArrayList<ArrayList<String>> set2){
		...
		for (int i = 0; i < set1.size(); i++) {
			...
			for (int j = 0; j < set2.size(); j++) {
				...
				if(intersectionsize>0)
					sensitivity=(double)intersectionsize/currentrowset1.size();
				
                outmatrix[i][j]=sensitivity;	
			}
	...
	protected double[][] calculateIndexMatrixJoinRowsColumns(ArrayList<ArrayList<String>> rowsset1,
		...
		return calculateSensitivityIndexMatrixConcatenatedRowsAndColumns(rowsset1, colsset1, rowsset2, colsset2);
	}
	
	
	 /**
 	 * Calculate sensitivity index matrix concatenated rows and columns.
 	 *
 	 ...
 	 * @return the double[][]
 	 */
 	public static double[][] calculateSensitivityIndexMatrixConcatenatedRowsAndColumns(ArrayList<ArrayList<String>> rowsset1,
	  		...
	  		for (int i = 0; i < rowsset1.size(); i++) {
	  			...
	  			for (int j = 0; j < rowsset2.size(); j++) {
	  				...
	  				int currentsetsize=currentrowset1.size()*currentcolsset1.size();
	  				
	  				double sensitivity=0;
	                
					if(intersectionsize>0)
						sensitivity=(double)intersectionsize/currentsetsize;
